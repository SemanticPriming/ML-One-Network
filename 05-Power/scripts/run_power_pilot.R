...
# imagine), run a broad AIPE simulation sweep (5-500 participants, step
# 10, 10 sims/step) over every dataset in power_job_list.csv that has that
# variable and a filled-in n_per_item. The sweep starts as low as n=5 (not
# just 20) so it can actually locate the sample size where split-half
# reliability crosses its target, rather than assuming that crossing point
# is never below 20. The resulting power/reliability curves are used to
# pick a narrower start/stop sample-size window for the refine stage.
#
