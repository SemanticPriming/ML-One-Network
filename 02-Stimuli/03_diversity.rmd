...
build_context_space <- function(
  ...
  for (i in seq_along(batches)) {
    ...
    message(sprintf(
      "Batch %d/%d -> contexts=%d, vocab=%d | elapsed=%s ETA=%s",
      i, n_batches, quanteda::ndoc(C), quanteda::nfeat(C),
      .format_secs(elapsed), .format_secs(eta)
    ))
  ...
  H <- numeric(ncol(X))
  if (length(nonzero)) {
    message(sprintf("Computing per-term entropy weights for %d terms...", length(nonzero)))
    pb_H <- utils::txtProgressBar(min = 0, max = length(nonzero), style = 3)
    for (k in seq_along(nonzero)) {
      ...
      if (k %% 500 == 0 || k == length(nonzero)) utils::setTxtProgressBar(pb_H, k)
    }
    close(pb_H)
  }
  ...
  message(sprintf("Running truncated SVD (k=%d) over %d contexts x %d terms...", k, nrow(X), ncol(X)))
  svd_t0 <- Sys.time()
  sv <- irlba::irlba(X, nv = k, nu = k)
  message(sprintf("SVD done in %s", .format_secs(as.numeric(difftime(Sys.time(), svd_t0, units = "secs")))))
...
# OpenSubtitles2024 ships as one monolithic file per language (see
# 01_subs_count.rmd), but build_context_space() batches file-by-file and
# reads/tokenizes each one whole before chunking it into contexts -- handing
# it a single multi-million-line file means it tries to tokenize the entire
# corpus as one document in one shot, which is what was hanging. Split it
# into many smaller chunk files once (streamed, so this doesn't itself
# require holding the whole corpus in memory) and read those instead.
raw_txt_path <- paste0(path, "/", language, ".txt")
chunks_dir <- paste0(path, "/chunks/")
chunk_lines <- 2000  # lines per chunk file -- roughly one "document" worth

if (!dir.exists(chunks_dir) || length(list.files(chunks_dir, pattern = "\\.txt$")) == 0) {
  dir.create(chunks_dir, recursive = TRUE, showWarnings = FALSE)

  con <- file(raw_txt_path, "rt", encoding = "UTF-8")
  chunk_i <- 0L
  repeat {
    lines <- readLines(con, n = chunk_lines, encoding = "UTF-8")
    if (length(lines) == 0) break
    chunk_i <- chunk_i + 1L
    writeLines(lines, sprintf("%s%s_chunk_%05d.txt", chunks_dir, language, chunk_i),
               useBytes = TRUE)
  }
  close(con)
}

file_list <- list.files(path = chunks_dir,
                        pattern = "\\.txt$",
                        full.names = TRUE,
                        recursive = TRUE)

# read the word list straight from the finalized trials rather than a
# separate item_data intermediate -- keeps this in sync with whatever
# uk.Rmd (and its per-language equivalents) actually use as the item set
my_keep <- rio::import(paste0("../01-Translation/05_final_languages/",
                              language, "/", language, "_trials_final.csv")) %>%
  filter(type == "related") %>%
  select(all_of(c(paste0(language, "_cue"), paste0(language, "_target")))) %>%
...
write.csv(sem_hoff, paste0(out_dir, language, "_semantic_diversity.csv"))
```