...
Set up this section to control the downloads for the files that are required below. 

Note that this process was completed with the original SPAML project 
within the materials folder: https://github.com/SemanticPriming/SPAML. We have copied over 
the file to show the entire process for this project, albeit this was completed before 
SPAML part 2 was started. 

Note that the links for the data download from subs2vec were validated June 2026,
but they do require an account to log in and download now, which does make this
code not fully reproducible. They are available at: https://github.com/jvparidon/subs2vec

Three languages did not use subs2vec and instead required hand-created vector files:
Japanese (`ja`), Chinese Simplified (`zh_cn`), and Chinese Traditional (`zh_tw`) each
need a word2vec-style CSV placed in the appropriate `subs_vec/{lang}/` subfolder.
Vietnamese (`vi`) subs_count and subs_vec zip files were also pre-populated rather than
downloaded. See `updatenotes.md` in the project root for exact filenames and locations.
...
This section imports the subtitles data from the Open Subtitles website and breaks down the 
files into smaller components for processing. The second function imports information from the 
`subs2vec` project to leverage preprocessed data. 
