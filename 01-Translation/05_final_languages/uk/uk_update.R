...
DF <- import("01-Translation/05_final_languages/uk/uk_translated_final.xlsx")
cues <- DF |> 
  filter(is.na(uk_fake_cue_trans)) |> 
  select(uk_cue_trans) |> 
  pull()
targets <- DF |> 
  filter(is.na(uk_fake_target_trans)) |> 
  select(uk_target_trans) |> 
  pull()
...
uk_patterns <- read.hyph.pat("01-Translation/02_stimuli_translation/hyphen/hyph-uk.tex", "uk")

# create fake words based on cue
uk_fake_update <- get_fake(wordlist = c(DF$uk_cue_trans,DF$uk_target_trans), #possibles
                          language_hyp = uk_patterns, #hyphen rules
                          replacewords = c(cues,targets)) #your words

DF <- DF |> 
  left_join(
    uk_fake_update |> select(original_word, replacement_word),
    by = c("uk_cue_trans" = "original_word")
  ) |> 
  rename("uk_fake_cue_update" = "replacement_word")

DF <- DF |> 
  left_join(
    uk_fake_update |> select(original_word, replacement_word),
    by = c("uk_target_trans" = "original_word")
  ) |> 
  rename("uk_fake_target_update" = "replacement_word") 

DF <- DF %>%
  mutate(
    uk_fake_cue_trans = coalesce(uk_fake_cue_update, uk_fake_cue_trans),
    uk_fake_target_trans = coalesce(uk_fake_target_update, uk_fake_target_trans)
  ) %>%
  select(-ends_with("_update"))

export(DF, "01-Translation/05_final_languages/uk/uk_translated_final.xlsx")
