...
knitr::opts_knit$set(root.dir = "matched_stimuli")
```
...
```{r}
# Path where per-language files from 01_translation_stimuli.Rmd and 02_finalize_stimuil.Rmd were written
# (relative to matched_stimuli/, which is the working directory when knitting this file)
...
import("fi_matched.csv") %>% 
  ...
  left_join(
    import(paste0(out_path, "/fi/fi_trials_final.csv")) %>%
...
import("hi_matched.csv") %>% 
  ...
  select(target_word_unique, word_combo_related, cue_word_related) %>% 
  left_join(
    import(paste0(out_path, "/hi/hi_trials_final.csv")) %>%
  filter(type == "unrelated") %>% 
  mutate(target_word_unique = make.unique(hi_target),
...
import("sl_matched.csv") %>% 
  ...
  select(target_word_unique, word_combo_related, cue_word_related) %>% 
  left_join(
    import(paste0(out_path, "/sl/sl_trials_final.csv")) %>%
  filter(type == "unrelated") %>% 
  mutate(target_word_unique = make.unique(sl_target),
...
# uk went through 05_final_languages, not the *_matched.csv step above,
# so both the related and unrelated side come from uk_trials_final.csv
import(paste0(out_path, "/uk/uk_trials_final.csv")) %>%
  filter(type == "related") %>%
  mutate(target_word_unique = make.unique(uk_target)) %>%
  mutate(word_combo_related = paste0(uk_cue, uk_target)) %>%
  dplyr::rename(cue_word_related = uk_cue) %>%
  select(target_word_unique, word_combo_related, cue_word_related) %>%
  left_join(
    import(paste0(out_path, "/uk/uk_trials_final.csv")) %>%
  filter(type == "unrelated") %>%
  mutate(target_word_unique = make.unique(uk_target),
         word_combo_unrelated = paste0(uk_cue, uk_target)) %>%
  dplyr::rename(cue_word_unrelated = uk_cue) %>%
  select(target_word_unique, word_combo_unrelated, cue_word_unrelated),
  by = "target_word_unique"
  ) %>%
  export("uk_trial_unique.csv")
```
...
temp2 <- import(paste0(out_path, "/ar/ar_trials_final.csv")) %>%
  #filter(type == "unrelated") %>% 
